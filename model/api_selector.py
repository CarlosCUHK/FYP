...
Do not use APIs that are not listed here (Don't make up any APIs by yourself, the following are the only APIs you can use).
...
Note, if the API path contains "{{}}", it means that it is a variable and you should replace it with the appropriate value. For example, if the path is "/users/{{user_id}}/tweets", you should replace "{{user_id}}" with the user id. "{{" and "}}" cannot appear in the url. In most cases, the id value is in the background or the API response. Just copy the id faithfully. If the id is not in the background, instead of creating one, call other APIs to query the id. For example, before you call "/users/{{user_id}}/playlists", you should get the user_id via "GET /me" first. Another example is that before you call "/person/{{person_id}}", you should get the movie_id via "/search/person" first. You should give a Thought considering the validity of your plan before giving it (check if the background information required are obtained).


Example 1:
Background: No background
User query: search for movies produced by DreamWorks Animation
API calling 1: Thought: I need DreamWorks Animation's id to search its movies 
	       Plan: GET /search/company to get the id of DreamWorks Animation
API response: DreamWorks Animation's company_id is 521
Instruction: Continue. Search for the movies produced by DreamWorks Animation
API calling 2: Thought: DreamWorks Animation's company_id is 521, I can use it to get the movies 
	       Plan: the GET /discover/movie to get the movies produced by DreamWorks Animation
API response: Puss in Boots: The Last Wish (id 315162), Shrek (id 808), The Bad Guys (id 629542), ...
...
Always think comprehensively and seek clarification from users whenever the user's query is not clear enough for you to formulate API calling. If you require additional information or face uncertainty, start the conversation with "I need user's clarification" and then express the challenges explicitly. Another thing to be noted is that do not ask user's for information related to id. 

You only need to output API calling. Instructions and API response will be generated by other models and you should not make up it by yourself. API calling should follow the format "action endpoint purpose" (the purpose should stick to the given plan)
...
class APISelector(Chain):
    ...
    def __init__(self, llm: BaseLLM, scenario: str, api_spec: ReducedOpenAPISpec) -> None:
        ...
        api_selector_prompt = PromptTemplate(
            ...
            input_variables=["plan", "background", "agent_scratchpad", "leader_response"],
        )
    ...
    def _call(self, inputs: Dict[str, Any]) -> Dict[str, str]:
        ...
        api_selector_chain_output = api_selector_chain.run(plan=inputs['plan'], background=inputs['background'], agent_scratchpad=scratchpad, leader_response=inputs["leader"], stop=self._stop)
