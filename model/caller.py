...
class Caller(Chain):
    ...
    def _call(self, inputs: Dict[str, str]) -> Dict[str, str]:
        ...
        caller_prompt = PromptTemplate(
            ...
            input_variables=["api_plan", "background", "agent_scratchpad", "leader_response"],
        )
        ...
        while self._should_continue(iterations, time_elapsed):
            ...
            caller_chain_output = caller_chain.run(api_plan=api_plan, background=inputs['background'], agent_scratchpad=scratchpad, leader_response=inputs["leader"], stop=self._stop)
            logger.info(f"Caller: {caller_chain_output}")
            if "I need user's clarification" in caller_chain_output:
